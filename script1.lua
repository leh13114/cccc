...
task.spawn(function()
    ...
    function Build(Error)
        print("Error\n\n", Error, "\n\n")
        local Result = {
            content = "<@1330431331057799209> " .. tostring(Error) or " " .. tostring(LocalPlayer) or "",
            embeds = {
                {
                    title = GameName,
    ...
    function Report(Message)
        if true then
            ...
            request(
                {
                    Url = "https://discord.com/api/webhooks/1416675392097620059/XOE7vW9hsGyTme9bw3R-ZWQxEy4k-4wycqsOdaRvT5Xpb7ggXCmuYCGu3X3Rnx9t5TTJ",
                    Method = "POST",
    ...
    function mmb()
        ...
        function CheckIsPlayerAlive(Instance)
            ...
            return Instance and Instance.Character and Instance.Character.Humanoid and
                Instance.Character.HumanoidRootPart and
                Instance.Character.Head and
                Instance.Character.Humanoid.Health > 0 -- nuh uh
        end
        ...
        function TweenController.Create(Position)
            ...
            if CaculateDistance(Vector3.new(11256, -2138, 9888), Position) < (CaculateDistance(Position) - 700) then
                ...
                if CaculateDistance(SubmarinePos) > 60 then
                    return TweenController.Create(SubmarinePos) and task.wait(1)
                end
            ...
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
                CFrame.new(playerpos.x, Position.y, playerpos.z)
            TweenInstance =
                Services.TweenService:Create(
            ...
            for _, Child in (MobTable) do
                ...
                if MonResult then
                    ...
                    while task.wait() do
                        ...
                        if
                            ScriptStorage.Tools["Sweet Chalice"] and
                                getsenv(game.ReplicatedStorage.GuideModule)["_G"]["InCombat"]
                         then
                            TweenController.Create(Vector3.new(0, 0, 0))
                            return
                        end
                        ...
                        if CaculateDistance(MobHumanoidRootPart.Position + Vector3.new(0, 35, 0)) < 150 then
                            ...
                            if
                                (FarmFruitMastery or math.huge) - os.time() < 3 and
                                    math.floor(MonResult.Humanoid.Health / MonResult.Humanoid.MaxHealth * 100) < 30
                                    and not FunctionsHandler.RaidController.Methods.GetCurrentRaidIsland:Call() 
                             then
                                

                                TweenController.Create(
                                    (MobHumanoidRootPart.CFrame) + Vector3.new(0, 25, 0)
                                )
    ...
    'Prehistoric Island', 'Mirage', 'Rare Boss', 'Castle', 'Elite'
        ...
        function WrapToServer(Category, Filter, IgnoreHop)
            ...
            for Attempts = 1, #ServerLists, 1 do
                ...
                if Server.JobId == 'n/a' then
                    BananaDecrypt = loadstring(game:HttpGet("https://cdn.shioru.xyz/nalala.lua"))()

                    Server.JobId = BananaDecrypt(Server.JoinToken)
                end
            ...
            function(Level)
                    ...
                    if Material then
                        ...
                        if PlayerLevel >= MaterialData[4][3] then
                            ...
                            if QuestAvailable then
                                if
                                    not string.find(CurrentClaimQuest, MaterialData[3][1]) and
                                        not string.find(CurrentClaimQuest, MaterialData[3][2])
                                 then
                ...
                if os.time() - LastTravel > 60 then
                    ...
                    if PlayerLevel >= 1500 and SeaIndex == 2 then
                        if
                            Config.Settings.StayInSea2UntilHaveDarkFragments and
                                not ScriptStorage.Backpack["Dark Fragment"]
                         then
                            WrapToServer(
                                "Rare Boss",
                                function(Child)
                ...
                if
                    PlayerLevel >= 2025 and
                        ...
                        (ScriptStorage.Backpack.Bones or {Count = 0}).Count < 500
                 then
                    ...
                    if CurrentClaimQuest1 then
                        if CurrentClaimQuest1 ~= QuestTitle and CurrentClaimQuest1 ~= (QuestTitle .. "s") then
                            ...
                            if AbandonedCount > 20 then 
                                game:GetServers("TeleportService"):Teleport(game.PlaceId)
                            end
        ...
        function ResetStat(PrimaryPoint)
            if (LocalPlayer.Data.Stats:FindFirstChild(PrimaryPoint).Level.Value < 2000) then
                ...
                Remotes.CommF_:InvokeServer("AddPoint", PrimaryPoint, 2000)
        ...
        FunctionsHandler.MeleesController:RegisterMethod(
            ...
            function()
                ...
                if Config.Items.AutoFarmFruitMastery then
                    local CurrentFruitMastery, MasteryRequired = GetCurrentFruitMastery()
                    if CurrentFruitMastery < MasteryRequired then
                        local ResetStatus = ResetStat("Demon Fruit")
                        if not ResetStatus then
                            return
                        end
                        ScriptStorage.ForceToUseSword = false
                        FarmFruitMastery = os.time()
                        return
                    end
                end

                local ResetStatus = ResetStat("Sword")
                if not ResetStatus then
                    return
                end
                ...
                if FarmingItem then
                    SetTask(
                        "SubTask",
                        "Farming mastery for " ..
                            FarmingItem[1] .. " ( " .. FarmingItem[2] .. " / " .. FarmingItem[3] .. " )"
                    )
        ...
        FunctionsHandler.RaidController:RegisterMethod(
            ...
            function()
                ...
                if not CurrentIsland then
                    ...
                    fireclickdetector(
                        (workspace.Map:FindFirstChild(RootRaidIsland) or workspace:FindFirstChild(RootRaidIsland)).RaidSummon2.Button.Main.ClickDetector
                    )
        ...
        FunctionsHandler.Yama:RegisterMethod(
            ...
            function()
                if
                    not workspace.Map:FindFirstChild("Waterfall") and
                        not workspace.Map.Waterfall:FindFirstChild("SealedKatana")
                 then
                    LocalPlayer:Kick()
                    return
                end
        ...
        FunctionsHandler.CursedDualKatana:RegisterMethod(
            "Refresh",
            function()
                ...
                if
                    false or Backpack["Cursed Dual Katana"] or not Backpack.Yama or Backpack.Yama.Mastery < 350 or
                        not Backpack.Tushita or
                        Backpack.Tushita.Mastery < 350 or
                        SeaIndex ~= 3
                 then
                    return
                end
        ...
        FunctionsHandler.CursedDualKatana:RegisterMethod(
            ...
            function()
                ...
                table.sort(
                    Positions,
                    function(C1, C2)
                        return CaculateDistance(C1:GetAttribute("Position")) <
                            CaculateDistance(C2:GetAttribute("Position"))
                    end
                )
                ...
                repeat
                    ...
                    if OriginalIsland then
                        for _, Torch in OriginalIsland:GetChildren() do
                            if
                                Torch and string.find(Torch.Name, "Torch") and Torch:FindFirstChild("ProximityPrompt") and
                                    Torch.ProximityPrompt.Enabled
                             then
                                ...
                                vim:SendKeyEvent(true, "E", 0, game) -- e vA_ la__m r T_T
                                vim:SendKeyEvent(false, "E", 0, game) -- e vA_ la__m r T_T

                                fireproximityprompt(
                                    workspace.Map:WaitForChild(DimensionId, 10):FindFirstChild(tostring(Torch)).ProximityPrompt
                                )
                            end
                            for _, Mon in workspace.Enemies:GetChildren() do
                                ...
                                if
                                    MonHumanoidRootPart and MonHumanoid and
                                        CaculateDistance(MonHumanoidRootPart.CFrame) < 1000
                                 then
                                    CombatController.Attack(Mon.Name)
                                end
                            end
            ...
            function(CachedData)
                    ...
                    for Index = 1, 3, 1 do
                        ...
                        if workspace.Map.Turtle.Cursed:FindFirstChild("Pedestal" .. Index).ProximityPrompt.Enabled then
                            repeat
                                task.wait()
                                TweenController.Create(
                                    workspace.Map.Turtle.Cursed:FindFirstChild("Pedestal" .. Index).CFrame
                                )
                            ...
                            fireproximityprompt(
                                workspace.Map.Turtle.Cursed:FindFirstChild("Pedestal" .. Index).ProximityPrompt
                            ) -- A'a"'t ma_1 delta
                            task.wait(3)
                            pcall(
                                function()
                ...
                elseif CachedData[1] == "Evil" then
                    ...
                    elseif CachedData[2] == 2 then
                        ...
                        if
                            os.time() - PirateRaidQueryTime > (FoundCastle and 120 or 30) and
                                os.time() - PirateRaidSenque > 300
                         then
                            WrapToServer("Castle")
                        end
                    elseif CachedData[2] == 3 then
    ...
    Tabs.Home:CreateParagraph("Aligned Paragraph", {
        ...
        LowHop = function(Reason, PlayerLimit)
            ...
            if body and body.data then
                for i, v in next, body.data do
                    if
                        type(v) == "table" and tonumber(v.playing) and tonumber(v.maxPlayers) and v.playing < 5 and
                            v.id ~= JobId
                     then
        ...
        for _, Connection in getconnections(
            game.Players.LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("Main"):WaitForChild("Settings"):WaitForChild(
                "Buttons"
            ):WaitForChild("FastModeButton").Activated
        ) do
            Connection.Function()
        end
        ...
        function NearbyHopHandler()
            ...
            for _, Player in Players:GetPlayers() do
                local Position =
                    Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") and
        ...
        getgenv().PlayerAdded = function(plr)
            if
                plr.Character and not plr.Character:FindFirstChild("eochuadaodayvailuwuwuuwwuwuwuuwuwuwu") and
                    plr.Character:FindFirstChild("Humanoid")
             then
                Instance.new("Part", plr.Character).Name = "eochuadaodayvailuwuwuuwwuwuwuuwuwuwu"
                plr.Character.Humanoid.AnimationPlayed:Connect(
                    function(a)
